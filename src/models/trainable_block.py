    ...
    def __init__(
        ...
        if head_type == "identity":
            ...
            if in_features is None:
                if in_channels is None or input_size is None:
                    raise ValueError(
                        "Either `in_features` or (`in_channels` and `input_size`) must be specified."
                    )
                in_features = in_channels * input_size * input_size
            else:
                if in_channels is not None and input_size is not None:
...
class TrainableBlockConfig:
    ...
    def __init__(
        ...
        if self.head_type not in ["linear", "mlp", "conv", "conv_2", "identity"]:
        ...
        if out_features is None and (out_channels is None or input_size is None):
            raise ValueError(
                "Either `out_features` or (`out_channels` and `input_size`) must be specified."
            )
...
class TrainableBlock1d(nn.Module):
    ...
    def __init__(
        ...
        if self.block_cfg.use_normalization:
            assert self.block_cfg.out_features is not None
        ...
        if self.loss_cfg.loss_type == "cross_entropy":
            if (
                self.block_cfg.head_type == "identity"
                and self.block_cfg.out_features != self.block_cfg.num_classes
            ):
                raise ValueError(
                    "Output dimension must be the same as the number of classes."
                )
    ...
    def forward_with_loss(
        self, x: Tensor, y: Tensor, model_input: Tensor
    ) -> LayerForwardResult:
        output = self.forward(x)
        loss = self.loss_func.criterion(
            fx=self.head(output), x=x, y=y, model_input=model_input
        )
...
class TrainableBlock2d(nn.Module):
    ...
    def __init__(
        ...
        if self.block_cfg.out_channels is None:
            raise ValueError("Output channels must be specified for 2D data.")
        ...
        if self.block_cfg.head_type in ["linear", "mlp"]:
            ...
            assert (
                self.block_cfg.input_size is not None
            ...
            if self.loss_cfg.loss_type == "cross_entropy":
                raise ValueError(
                    "Convolutional head is not supported for cross-entropy loss."
                )
                ...
                raise ValueError(
                    "Convolutional head is not supported for supervised contrastive loss."
                )
            else:
                assert False, "Unreachable"
            ...
            if self.loss_cfg.loss_type == "cross_entropy":
                raise ValueError(
                    "Identity head is not supported for cross-entropy loss."
                )
            elif self.loss_cfg.loss_type == "similarity":
    ...
    @staticmethod
    def get_avg_pool(
        out_channels: int, input_size: int, dim_in_decoder: int
        ...
        if kernel_size_h > 1 or kernel_size_w > 1:
            ...
            avg_pool = nn.AvgPool2d(
                (kernel_size_h, kernel_size_w), padding=(padding_h, padding_w)
            )
        else:
            avg_pool = None
