...
class Model(nn.Module):
    ...
    def __init__(self, block, num_blocks, in_channel: int = 3):
        ...
        self.layer1 = self._make_layer(
            block, self.init_in_planes, num_blocks[0], stride=1
        )
        self.layer2 = self._make_layer(
            block, self.init_in_planes * 2, num_blocks[1], stride=2
        )
        self.layer3 = self._make_layer(
            block, self.init_in_planes * 4, num_blocks[2], stride=2
        )
        self.layer4 = self._make_layer(
            block, self.init_in_planes * 8, num_blocks[3], stride=2
        )
...
class SupConResNet(nn.Module):
    ...
    def __init__(
        self, model_name: str = "resnet50", head_type: str = "mlp", feat_dim: int = 128
    ):
        super().__init__()
